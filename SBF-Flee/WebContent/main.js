...
function update() {
    ...
    for (var i = 0; i < boidsAmount; i++) {
        flee(boids[i]);
        boids[i].body.onWorldBounds = new Phaser.Signal();
        boids[i].body.onWorldBounds.add(resetTarget,this);
    }
}

function initBoids() {
    for (var i = 0; i < boidsAmount; i++) {
        var randomPoint = new Phaser.Point(game.rnd.between(target.x - 30, target.x + 30), game.rnd.between(target.y -30, target.y + 30));
...
function resetTarget(boid) {
        ...
        boid.x = game.rnd.between(target.x - 100, target.x + 100);
        boid.y = game.rnd.between(target.y - 100, target.y + 100);
        fleeCounter++;
}

function render() {
    game.debug.text("Flee times :"+ fleeCounter, 10, 20);
}
